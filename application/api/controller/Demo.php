 ...
 * @ApiParams   (name="endDate", type="string", required=false, description="计息结束日期，格式：YYYY-MM-DD，默认为当前日期")
    ...
    /**
     * 检测用户2025年8月16日利息重复发放
     *
     * @ApiTitle    (检测用户2025年8月16日利息重复发放)
     * @ApiSummary  (检测指定用户在2025年8月16日是否有重复发放的消费补贴金利息)
     * @ApiMethod   (POST)
     * @ApiRoute    (/api/demo/checkInterestDuplicate)
     * @ApiHeaders  (name=token, type=string, required=false, description="请求的Token")
     * @ApiParams   (name="user_id", type="integer", required=true, description="要检测的用户ID")
     * @ApiReturnParams   (name="code", type="integer", required=true, sample="1")
     * @ApiReturnParams   (name="msg", type="string", required=true, sample="检测完成")
     * @ApiReturnParams   (name="data", type="object", sample="{'检测结果':'string','重复记录':'array'}", description="返回数据")
     * @ApiReturn   ({
         'code':'1',
         'msg':'检测完成',
         'data': {
     ...
     */
    public function checkInterestDuplicate()
    {
        // 获取用户ID参数
        $userId = $this->request->param('user_id', 0, 'intval');
        ...
        // 设置检测日期范围（2025年8月16日 00:00:00 到 23:59:59）
        $checkDate = '2025-08-16';
        $startTime = strtotime($checkDate . ' 00:00:00');
        $endTime = strtotime($checkDate . ' 23:59:59');

        // 查询用户在指定日期的利息发放记录
        $interestLogs = Db::name('user_nxxfjlxlog')
            ->where([
                'user_id' => $userId,
        ...
        // 查询用户在指定日期的存单更新记录
        $depositUpdates = Db::name('nxxfjlist')
            ->where([
                'user_id' => $userId,
        ...
        // 分析重复发放情况
        $duplicateAnalysis = $this->analyzeInterestDuplicates($interestLogs, $depositUpdates, $checkDate, $userId);

        // 查询用户当前的利息余额
        $currentInterest = Db::name('user')->where('id', $userId)->value('nxxfjlx') ?: 0;
        ...
        $this->success('检测完成', [
            '用户信息' => [
                ...
                '当前利息余额' => $currentInterest
            ],
            '检测日期' => $checkDate,
            '重复发放检测' => $duplicateAnalysis,
            '建议操作' => $this->getRecommendations($duplicateAnalysis, $interestLogs, $currentInterest)
        ]);
    }
     ...
     * 分析利息重复发放情况
     * 
     * @param array $interestLogs 利息发放日志
     * @param array $depositUpdates 存单更新记录
     * @param string $checkDate 检测日期
     * @param int $userId 用户ID
     * @return array 分析结果
     */
    private function analyzeInterestDuplicates($interestLogs, $depositUpdates, $checkDate, $userId)
    {
        // 查询用户当前的有效存单数量（状态为1的存单）
        $validDeposits = Db::name('nxxfjlist')
            ->where([
                'user_id' => $userId,
                'status' => 1
            ])
        ...
        $analysis = [
            '总发放次数' => count($interestLogs),
            '总发放金额' => 0,
            '有效存单数量' => count($validDeposits),
            '重复发放风险' => '低'
        ];

        // 计算总发放金额
        foreach ($interestLogs as $log) {
            $analysis['总发放金额'] = bcadd($analysis['总发放金额'], $log['money'], 2);
        }

        // 检查异常情况
        if (count($interestLogs) > 24) {
            $analysis['异常情况'][] = '利息发放次数过多（超过24次），可能存在重复发放';
            $analysis['重复发放风险'] = '高';
        }

        if (count($interestLogs) > 0 && count($depositUpdates) == 0) {
            $analysis['异常情况'][] = '有利息发放但无存单更新，可能存在异常';
            $analysis['重复发放风险'] = '中';
        }
        ...
        $timeGroups = [];
        foreach ($interestLogs as $log) {
            $timeKey = date('H:i', $log['createtime']);
            if (!isset($timeGroups[$timeKey])) {
                $timeGroups[$timeKey] = [];
            }
            $timeGroups[$timeKey][] = $log;
        }

        foreach ($timeGroups as $time => $logs) {
            if (count($logs) > 1) {
                $analysis['异常情况'][] = "时间点 {$time} 有 " . count($logs) . " 次发放，可能存在重复";
                $analysis['重复发放风险'] = '高';
            }
        }
        ...
        foreach ($memoCounts as $memo => $count) {
            if ($count > 1) {
                $analysis['异常情况'][] = "备注 '{$memo}' 出现 {$count} 次，可能存在重复发放";
                $analysis['重复发放风险'] = '中';
            }
        }

        // 核心检测逻辑：发放次数应该等于有效存单数量
        $expectedCount = count($validDeposits);
        $actualCount = count($interestLogs);
        ...
        // 计算应该发放的总金额（有效存单数量 × 日利息）
        $expectedTotalAmount = 0;
        foreach ($validDeposits as $deposit) {
            $expectedTotalAmount = bcadd($expectedTotalAmount, $deposit['interest'], 2);
        }
        
        // 计算多发放的金额
        $overpaidAmount = 0;
        if ($actualCount > $expectedCount) {
            $overpaidAmount = bcsub($analysis['总发放金额'], $expectedTotalAmount, 2);
        }
        
        // 添加关键统计信息
        $analysis['关键统计'] = [
            '实际发放笔数' => $actualCount,
            '应该发放笔数' => $expectedCount,
            '实际发放总金额' => $analysis['总发放金额'],
            '应该发放总金额' => $expectedTotalAmount,
            '多发放金额' => $overpaidAmount > 0 ? $overpaidAmount : 0,
            '多发放笔数' => max(0, $actualCount - $expectedCount)
        ];
     ...
     * @param array $interestLogs 利息发放日志
     * @param float $currentInterest 当前利息余额
     * @return array 建议操作
     */
    private function getRecommendations($analysis, $interestLogs, $currentInterest)
    {
        $recommendations = [];

        if (isset($analysis['重复发放风险'])) {
            if ($analysis['重复发放风险'] === '高') {
                $recommendations[] = '存在重复发放，建议立即停止该用户的利息发放';
                $recommendations[] = '检查定时任务是否异常运行';
            }

            if ($analysis['重复发放风险'] === '中') {
                $recommendations[] = '建议进一步检查该用户的存单状态';
            }
        }

        // 基于存单数量的建议
        if (isset($analysis['总发放次数']) && isset($analysis['有效存单数量'])) {
            if ($analysis['总发放次数'] == 0 && $analysis['有效存单数量'] > 0) {
                $recommendations[] = '该用户有有效存单但无利息发放，可能存在定时任务异常';
            }

            if ($analysis['总发放次数'] > 0 && $analysis['有效存单数量'] == 0) {
                $recommendations[] = '该用户无有效存单但有利息发放，存在严重异常，需要立即排查';
            }

            if ($analysis['总发放次数'] == $analysis['有效存单数量']) {
                $recommendations[] = '发放次数与存单数量一致，属于正常情况';
            }
        }
