 ...
 * @icon fa fa-circle-o
 */
class CouponExchangeNewcash extends Backend
{
    ...
    {
        parent::_initialize();
        $this->model = new \app\admin\model\CouponExchangeNewcash;
    ...
    public function index()
    {
        ...
        if ($this->request->isAjax()) {
            ...
            $list = Db::name('coupon_exchange_newcash')
                ->alias('c')
                ...
                ->limit($offset, $limit)
                ->select();
            $total = Db::name('coupon_exchange_newcash')
                ->alias('c')
                ->join('user b', 'c.user_id = b.id', 'left')
                ->where($w)
                ->field('c.*,b.nickname,b.id as user_id')
    ...
    public function edit($ids = null)
    {
        ...
        if ($this->request->isPost()) {
            ...
            $w1 = [];
            $w1['id'] = array("eq", $ids);
            $r1 = Db::name("coupon_exchange_newcash")->where($w1)->find();
            if (!$r1) {
                $this->error("未查询到信息");
            }
        ...
        if (request()->isPost()) {
            ...
            foreach ($ids as $key => $value) {
                ...
                if ($r1['status'] == 2 && $data['status'] == 1) {
                    ...
                    Db::name("coupon_exchange_newcash")->where($w1)->update(['status' => $data['status']]);
                }
    ...
    public function autoApproveAll()
    {
        if (request()->isPost()) {
            ...
            // 获取所有待审核的兑换申请总数
            $totalCount = Db::name("coupon_exchange_newcash")
            ...
            // 分批获取数据
            $pendingRecords = Db::name("coupon_exchange_newcash")
                ->where('status', 2) // 待审核状态
            ...
            try {
                foreach ($pendingRecords as $record) {
                    ...
                    if ($status == 1) {
                        ...
                        Db::name("user_money_log")->insert($detailed_data);
                        Db::name("coupon_exchange_newcash")->where('id', $record['id'])->update(['status' => $status]);
                        $successCount++;
                    } else {
                        // 驳回申请
