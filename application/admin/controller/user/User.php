...
class User extends Backend
{
    ...
    public function index()
    {
        ...
        if ($this->request->isAjax()) {
            ...
            // 检查是否是异步加载详细数据的请求
            if ($this->request->request('action') === 'loadDetails') {
            ...
            foreach ($list as $k => $v) {
                ...
                // 添加异步加载标识
                $v->details_loaded = false;
                $v->loading_details = false;
            }
...
//   `upid3` bigint(255) DEFAULT '0' COMMENT '上级ID',
     ...
     * 异步加载用户详细统计信息
     */
    public function asyncUserStats()
    {
        if ($this->request->isAjax()) {
            ...
            foreach ($userIds as $userId) {
                ...
                // 获取直推人数
                $zhituinum = \app\api\model\User::where(['upid' => $userId, 'is_buy' => 1])->count();
                
                // 获取全部下级数量
                $allnum = User::where('upid|upid2|upid3', $userId)->count();
                
                // 获取累计消费
                $allbuy = \app\api\model\Order::where('user_id', $userId)->sum('price') ?: 0;
                
                // 获取IP地理位置
                $user = User::where('id', $userId)->field('loginip')->find();
                $ipname = '未知';
                if ($user && $user['loginip']) {
                    try {
                        $csmip = \addons\csmip\library\Csmip::getInstance();
                        $region = $csmip->getRegion($user['loginip']);
                        $ipname = $region->province . '-' . $region->city;
                    } catch (\Exception $e) {
                        $ipname = '解析失败';
                    }
                }
    ...
    {
        $userIds = $this->request->request('user_ids');
        ...
        foreach ($userIds as $userId) {
            try {
                $userDetail = $this->getUserDetailData($userId);
                $details[$userId] = $userDetail;
            } catch (\Exception $e) {
                $details[$userId] = ['error' => $e->getMessage()];
            }
        }
    ...
    private function getUserDetailData($userId)
    {
        ...
        // 1. 推荐关系数据
        $detail['zhituinum'] = \app\api\model\User::where(['upid' => $userId, 'is_buy' => 1])->count();
        $detail['allnum'] = User::where('upid|upid2|upid3', $userId)->count();
        
        // 2. 累计消费数据
        $detail['allbuy'] = \app\api\model\Order::where('user_id', $userId)->sum('price') ?: 0;
        
        // 3. 身份等级数据
        $detail['levename'] = \app\api\model\Level::where('id', $user->level)->value('name') ?: '无身份';
        
        // 4. IP地理位置数据（使用缓存优化）
        $detail['ipname'] = $this->getCachedIpLocation($user->loginip);
        
        // 5. 用户组信息
        $detail['group'] = $user->group ? $user->group->name : '默认组';
     ...
     * 获取缓存的IP地理位置信息
     */
    private function getCachedIpLocation($ip)
    {
        if (empty($ip)) {
            return '未知';
        }
        ...
        try {
            ...
            $location = $region->province . '-' . $region->city;
            
            // 缓存结果（24小时）
            cache($cacheKey, $location, 86400);
