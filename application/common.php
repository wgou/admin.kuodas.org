...
if (!function_exists('set_redis')) {
    ...
    function set_redis($gname,$name,$value, $time = 60)
    {
        Cache::store('redis')->handler();
        Cache::store('redis')->hset($gname,$name,serialize($value));
        Cache::expire($gname,$time);
        return true;
    }
}
...
if (!function_exists('get_redis')) {
    //防重复点击逻辑，开启redis数据  毫秒级防重复
    function get_redis($gname,$name)
    {
        return unserialize(Cache::store('redis')->hget($gname,$name));
    }
}

if (!function_exists('put_redis')) {
    //防重复点击逻辑，开启redis数据  毫秒级防重复
    function put_redis($key,$value)
    {
        Cache::store('redis')->handler();
        Cache::store('redis')->rpush($key,serialize($value));
    }
}

if (!function_exists('del_redis')) {
    //防重复点击逻辑，开启redis数据  毫秒级防重复
...
function api_paginate($table,$w,$p,$per_page = 20)
{
    $p = $p == '' ? 1 : $p;
    $items_per_page = $per_page;
    $offset = ($p - 1) * $items_per_page;
    $total = Db::name($table)->where($w)->count('id');
    $info = Db::name($table)
        ->where($w)
        ->order('id desc')
...
function payment_log($name,$data){
    $log = date('Y-m-d H:i:s') . '---' . json_encode($data) . PHP_EOL;
    $path = 'pay_callback_log';
    if (!is_dir($path)) {
        mkdir($path, 0777, true);
    }
    file_put_contents($path . '/' . date('Y-m-d') . '_'.$name.'回调数据' . '.log', $log, FILE_APPEND);
}

function pay_log($name,$data){
    $log = date('Y-m-d H:i:s') . '---' . json_encode($data) . PHP_EOL;
    $path = 'pay_log';
    if (!is_dir($path)) {
        mkdir($path, 0777, true);
